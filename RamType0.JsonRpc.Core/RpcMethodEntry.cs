...
namespace RamType0.JsonRpc
{
    ...
    using RamType0.JsonRpc.Internal.Emit;
    ...
    public abstract class RpcMethodEntry : RpcAsyncMethodEntry
    {
            ...
            where T : notnull, Delegate
        {
            ...
            {
                return RpcDelegateEntryFactoryCache<T>.Instance.CreateAsyncMethodEntry(d, exceptionHandler);
            }
            else if (d.Method.IsStatic)
            {
                return RpcStaticMethodEntryFactoryCache<T>.Instance.CreateAsyncMethodEntry(d, exceptionHandler);
            }
            else
            {
                return RpcInstanceMethodEntryFactoryCache<T>.Instance.CreateAsyncMethodEntry(d, exceptionHandler);
            }
        ...
        where TModifier : notnull, IMethodParamsModifier<TParams>
    {
        ...
        public override ArraySegment<byte> ResolveRequest(ArraySegment<byte> parametersSegment, ID? id)
        {
            ...
            }
            {
                ...
                {
                    if (default(TResult)! == null && result is null)
                    {
